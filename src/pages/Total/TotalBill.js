...
function TotalBill(){
    ...
    const updateBill = (id) =>{
       axios.put("http://localhost:8080/api/bill/updateBill/" + id + "/" + userForm.values.nameUser + "/" + userForm.values.password).then(res =>{
        alert("update bill")
        axios.get("http://localhost:8080/api/bill")
        .then(response => {
          setBill(response.data);
                  ...
                  {bill.map(item => (
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                      ...
                      <td class="px-6 py-4 text-right">
                       <button onClick={(id) => updateBill(item.id)}>Confirm</button>
                      </td>
                    </tr>
                  ))}
                </>}
